...
import { claimLoserUsdc as refundUsdcToLoser } from '../lib/escrow'
                    ...
                    {!bid.is_winner && !bid.refunded && !bid.refund_pending && (
                      <button
                        onClick={async (e) => {
                          e.stopPropagation()
                          try {
                            await refundUsdcToLoser(bid.id, walletAddress, bid.amount ?? 0, 'phantom')
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        try {
          ...
          const res = await fetch(`${import.meta.env.VITE_BACKEND_URL || 'http://localhost:8000'}/api/creator-sales?wallet=` + walletAddress)
