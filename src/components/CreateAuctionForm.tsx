...
import { mintAndDepositNftToEscrow } from "../lib/solana";
...
export function CreateAuctionForm() {
  const { connected, publicKey, adapter } = useWallet();
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const mint = await mintAndDepositNftToEscrow(
        publicKey.toString(),
        adapter
      );
      ...
      const res = await fetch(
        "http://localhost:8000/api/create-auction",
        {
          method: "POST",
          headers: {
