...
import { sendBidTransaction, explorerTxUrl, getSolBalance } from '../lib/solana'
...
export default function CreateAuctionPage({ isDemoMode = false }: CreateProps) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (balance > 0 && balance < MINIMUM_SOL_REQUIRED) {
        throw new Error(
          `Insufficient SOL. You have ${balance.toFixed(4)} SOL, need at least ${MINIMUM_SOL_REQUIRED} SOL.`
        )
      }

      // 3. Tx 1: Listing fee 0.001 SOL ke escrow (Phantom popup)
      ...
      // 4. Mint NFT di backend (no Phantom needed)
      setCreatingPhase('sign_mint')
      let nftMintAddress: string
      let mintTxSig: string
      try {
        const backendUrl = import.meta.env.VITE_BACKEND_URL || 'http://localhost:8000'
        const mintRes = await fetch(`${backendUrl}/api/mint-nft`, {
          method: 'POST',
        ...
        mintTxSig = mintData.txSignature

        if (!nftMintAddress || nftMintAddress.length < 32) {
          throw new Error('Mint address invalid - transaction may have failed on-chain.')
        }
      } catch (e: any) {
      ...
      // 5. MagicBlock health check (non-blocking)
      checkMagicBlockHealth().catch((e) => {
        console.warn('[MagicBlock] Health check failed (non-fatal):', e)
      })

      // 6. Insert ke Supabase via backend
      setCreatingPhase('saving')
      const backendUrl = import.meta.env.VITE_BACKEND_URL || 'http://localhost:8000'
      const auctionRes = await fetch(`${backendUrl}/api/create-auction`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...
          creator: publicKey,
          minBid: parseFloat(form.min_bid),
          currency: form.currency,
          durationHours: form.duration_hours,
        }),
      })
      const auctionData = await auctionRes.json()
      if (!auctionData.success) throw new Error(auctionData.error || 'Create auction failed')

      setCreatedId(auctionData.auctionId)
      setStep('done')
  ...
  const phaseLabel: Record<CreatingPhase, string> = {
    ...
    sign_listing_fee: 'Sign transaction - Listing fee (0.001 SOL)...',
    sign_mint: 'Minting NFT to escrow...',
  ...
  const phaseSubLabel: Record<CreatingPhase, string> = {
    ...
    sign_listing_fee: 'Phantom will pop up - confirm the 0.001 SOL transfer to escrow.',
    sign_mint: 'Minting your NFT on-chain and depositing to escrow. Please wait...',
    ...
    <div className="pt-[116px] sm:pt-[88px] pb-16 px-4 sm:px-6 max-w-2xl mx-auto">
      {isDemoMode && (
        <div className="mb-6 px-4 py-3 bg-amber-500/8 border border-amber-500/20 rounded-xl flex items-center gap-3">
          ...
          <p className="text-xs text-amber-400/90"><strong>Demo Mode</strong> - Connect a real wallet to create actual on-chain auctions.</p>
        </div>
      )}
            ...
            {step === 'form' ? (
                      ...
                      {!uploadedFile ? (
                        ...
                        >
                          ...
                          <div className="text-center">
                            ...
                            <p className="text-xs text-[rgba(255,255,255,0.35)] mt-1">JPG, PNG, GIF, WEBP, MP4, WEBM - Max 10MB image / 50MB video</p>
                          </div>
                          <input
                            ref={fileInputRef}
                            type="file"
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <div className="flex-1 py-3 rounded-xl text-sm font-bold border text-center bg-[var(--accent)] text-[#080808] border-[var(--accent)]">
                        USDC
                      </div>
                    </div>
                  </div>
              ...
              <div className="space-y-5">
                ...
                {mediaSource === 'upload' && uploadedFile && (
                  <div className="p-3 bg-[rgba(255,255,255,0.04)] border border-[rgba(255,255,255,0.07)] rounded-xl flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-[rgba(255,255,255,0.35)]">{(uploadedFile.size / 1024 / 1024).toFixed(2)} MB - Will upload when you launch</p>
                    </div>
                  </div>
                )}
                ...
                <div className="p-4 border border-[rgba(200,255,0,0.20)] rounded-xl space-y-2" style={{ background: 'rgba(200,255,0,0.04)' }}>
                  <p className="text-xs font-semibold text-[var(--accent)]">What happens when you launch</p>
                  <div className="space-y-1">
                    <p className="text-xs text-[rgba(255,255,255,0.50)]">1. Sign listing fee - 0.001 SOL to escrow (Phantom popup)</p>
                    <p className="text-xs text-[rgba(255,255,255,0.50)]">2. NFT minted and deposited to escrow automatically</p>
                    <p className="text-xs text-[rgba(255,255,255,0.50)]">3. Auction goes live - bids are sealed until it ends</p>
                  </div>
