...
// ─── NFT TRANSFER (escrow → winner) ──────────────────────────────────────────

/**
 * transferNftToWinner — transfer NFT dari escrow ke winner.
 * Dipanggil server-side (Supabase Edge Function) karena butuh escrow private key.
 * Di frontend ini hanya placeholder — actual transfer via edge function.
 */
...
/**
 * refundSolToLoser — refund SOL dari escrow ke loser.
...
/**
 * refundUsdcToLoser — refund USDC dari escrow ke loser.
...
/**
 * depositNftToEscrow — seller deposit NFT ke escrow wallet.
 * Winner sign tx ini saat create auction → NFT masuk escrow.
 */
...
// ─── CLAIM NFT (winner-initiated, user signs + pays SOL fee) ─────────────────

/**
 * claimNftFromEscrow — dipanggil oleh winner dari dashboard.
 *
 * Flow:
 *   1. Winner klik "Claim NFT" di dashboard
 *   2. Frontend build tx: transfer nominal SOL ke escrow sebagai "claim fee" (proof-of-intent)
 *      — ini yang muncul di popup wallet winner, biar ada yang di-sign
 *   3. Setelah tx confirmed → panggil Supabase Edge Function `claim-nft`
 *      yang punya escrow private key → transfer NFT dari escrow ke winner
 ...
 * - Ini yang bikin tombol "Claim" meaningful — user benar-benar sign sesuatu
 */
export async function claimNftFromEscrow(
  ...
  // ── Step 1: Winner signs a small SOL tx as on-chain proof-of-claim ────────
  // 5000 lamports = 0.000005 SOL — effectively just the tx fee
...
// ─── MINT NFT + DEPOSIT TO ESCROW ────────────────────────────────────────────

/**
 * mintAndDepositToken — mint NFT baru + deposit ke escrow (plan baru: on-chain only).
 *
...
export async function returnNftToCreator(
  ...
  const response = await fetch(`${import.meta.env.VITE_BACKEND_URL || 'http://localhost:8000'}/api/return-nft-to-creator`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
