...
import { sendBidTransaction, explorerTxUrl } from '../lib/solana'

type Currency = 'SOL' | 'USDC'
type DurationOption = 1 | 6 | 12 | 24 | 48 | 72
...
const DURATION_OPTIONS: DurationOption[] = [1, 6, 12, 24, 48, 72]
...
export default function CreateAuctionPage({ isDemoMode = false }: CreateProps) {
  ...
  // â”€â”€ Determine final media URL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

  const getMediaUrl = () => {
    if (mediaSource === 'url') return form.image_url.trim()
    return uploadPreview // local blob for preview, real URL after upload
  }
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      // Upload file if user chose upload mode and has a file
      ...
      // â”€â”€ On-chain: listing fee 0.001 SOL ke escrow (triggers Phantom popup) â”€â”€
      if (!provider) throw new Error('No wallet provider found')
      const listingFeeSig = await sendBidTransaction(publicKey, 0.001, provider)
      setTxSignature(listingFeeSig)

      await checkMagicBlockHealth()
      ...
      setCreatedId(data?.id || 'new')
      setStep('done')
      setForm(INITIAL_FORM)
      clearMedia()
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Failed to create auction. Please try again.')
      setStep('error')
    }
  }
    ...
    <div className="pt-[116px] sm:pt-[88px] pb-16 px-4 sm:px-6 max-w-2xl mx-auto">
        ...
        <div className="glass border border-[rgba(255,255,255,0.07)] rounded-2xl p-8 text-center space-y-5">
          ...
          <div>
            ...
            <p className="text-[rgba(255,255,255,0.40)] mt-2">
              Your sealed-bid auction is live. MagicBlock PER session initialized.
            </p>
          </div>
      ...
      {step === 'creating' && (
        <div className="glass border border-[rgba(255,255,255,0.07)] rounded-2xl p-12 text-center space-y-5">
          ...
          <p className="text-white font-semibold text-lg">
            {uploading ? 'Uploading media...' : 'Creating Auction...'}
          </p>
          <p className="text-[rgba(255,255,255,0.40)] text-sm">
            {uploading ? 'Uploading your NFT media to storage.' : 'Initializing MagicBlock PER session and saving auction.'}
          </p>
        </div>
      )}
      ...
      {step === 'error' && (
        <div className="glass border border-[rgba(255,255,255,0.07)] rounded-2xl p-8 text-center space-y-4">
          ...
          <p className="text-sm text-[rgba(255,255,255,0.40)]">{error}</p>
              ...
              <div className="space-y-5">
                <div className="p-5 bg-[rgba(255,255,255,0.05)] border border-[rgba(255,255,255,0.07)] rounded-xl space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-3 pt-2">
                    ...
                    <div className="p-3 bg-[#080808]/50 rounded-xl text-center">
                      <p className="font-mono font-bold text-white">{form.duration_hours}h</p>
                ...
                <div className="p-4 border border-[rgba(200,255,0,0.20)] rounded-xl" style={{ background: 'rgba(200,255,0,0.04)' }}>
                  <p className="text-xs font-semibold text-[var(--accent)] mb-1">MagicBlock PER Session</p>
                  <p className="text-xs text-[rgba(255,255,255,0.40)] leading-relaxed">
                    A Private Ephemeral Rollup session will be created. All bids are sealed and completely private until settlement.
                  </p>
                </div>
