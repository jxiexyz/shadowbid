...
// Helper: apakah auction ini "ended" secara efektif?
// Ended = status 'live' tapi end_time udah lewat, ATAU status memang 'ended'
const isEffectivelyEnded = (a: Auction) =>
  a.status === 'ended' || (a.status === 'live' && new Date(a.end_time) < new Date())

const isEffectivelyLive = (a: Auction) =>
  a.status === 'live' && new Date(a.end_time) >= new Date()
...
export function useAuctions(isDemoMode = false) {
  ...
  const filterAndSort = (
    ...
    if (status === 'live') {
      filtered = filtered.filter(isEffectivelyLive)
    } else if (status === 'ended') {
      filtered = filtered.filter(a => isEffectivelyEnded(a) || a.status === 'settled')
    } else if (status === 'settled') {
      filtered = filtered.filter((a) => a.status === 'settled')
    }
    // 'all' = no filter
