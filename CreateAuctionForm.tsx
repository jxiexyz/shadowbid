...
import { mintAndDepositToEscrow } from "../lib/mintAndDeposit";
...
export function CreateAuctionForm() {
  ...
  const handleCreate = async () => {
    if (!form.title || !form.minBid || !connected) return;
    setSubmitting(true);
    try {
      // 1. Mint NFT on-chain & deposit to escrow
      const mintAddress = await mintAndDepositToEscrow(
        { adapter: wallet?.adapter, publicKey },
        form.title,
        form.description
      );

      // 2. Send valid data to backend
      const res = await fetch("http://localhost:8000/api/create-auction", {
        method: "POST",
  ...
  if (submitted) {
    return (
      ...
      >
        ...
        >
          {form.title} is now live on Solana Devnet. Bidders will see your
          auction but not each other's bids.
        </p>
        <div
          style={{
            background: "var(--surface2)",
  ...
  return (
    ...
    >
      ...
      >
        Bids will be encrypted via MagicBlock PER. No one can see bid amounts
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        {form.title && form.minBid && (
          ...
          >
            Creating: <span style={{ color: "var(--accent)" }}>{form.title}</span> · Min bid:{" "}
            <span style={{ color: "var(--accent)" }}>{form.minBid} {form.currency}</span> · Duration:{" "}
            <span style={{ color: "var(--accent)" }}>{form.durationHours}h</span>
          </div>
        )}
        ...
        >
          {submitting ? "Minting NFT & Deploying..." : "Deploy Auction 🚀"}
        </button>
      </div>
    </div>
  );
