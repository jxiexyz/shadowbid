        ...
        <div className="border-b border-[rgba(255,255,255,0.07)]" style={{ background: 'rgba(8,8,8,0.90)', backdropFilter: 'blur(28px) saturate(180%)', WebkitBackdropFilter: 'blur(28px) saturate(180%)' }}>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 h-[60px] flex items-center justify-between gap-4">
            ...
            <button onClick={() => onNavigate('explore')} className="flex items-center gap-2.5 group flex-shrink-0">
              ...
              <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ filter: 'drop-shadow(0 0 8px rgba(200,255,0,0.45))' }}>
                {/* Background pill */}
                <rect width="30" height="30" rx="8" fill="#0f0f0f" />
                {/* Shadow layer - offset dark shape */}
                <path d="M8 20 L15 8 L22 20 Z" fill="rgba(200,255,0,0.18)" />
                {/* Main accent arrow/bid shape */}
                <path d="M6 21 L15 6 L24 21 L20.5 21 L15 11.5 L9.5 21 Z" fill="var(--accent, #c8ff00)" />
                {/* Inner cut - creates negative space suggesting hidden/sealed bid */}
                <path d="M15 16 L18 21 L12 21 Z" fill="#0f0f0f" />
                {/* Dot - the sealed bid indicator */}
                <circle cx="15" cy="13.5" r="1.2" fill="#0f0f0f" />
              </svg>
            ...
            <nav className="hidden md:flex items-center gap-0.5">
              {NAV_ITEMS.map(({ key, label }) => (
                ...
                >
                  {label}
                  {currentView === key && (
                    <span className="absolute bottom-[3px] left-1/2 -translate-x-1/2 w-4 h-[2px] rounded-full" style={{ background: 'var(--accent)', boxShadow: '0 0 6px var(--accent-glow)' }} />
                  )}
                </button>
              ))}
            </nav>
            ...
            <div className="flex items-center gap-2.5">
              ...
              {connected && publicKey ? (
                <div className="relative">
                  ...
                  >
                    <span className="w-[7px] h-[7px] rounded-full flex-shrink-0 animate-pulse_live" style={{ background: 'var(--live)', boxShadow: '0 0 8px var(--live)' }} />
