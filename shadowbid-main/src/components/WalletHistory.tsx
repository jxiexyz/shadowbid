 ...
 * Shows: Bids history, NFTs received, Refunds, Creator sales — all with Proof TX links.
 *
...
// ─── SECTION: Bids History ────────────────────────────────────────────────────

function BidsSection({ bids, network }: { bids: BidRow[]; network: string }) {
  return (
    <div>
      <SectionHeader
...
// ─── SECTION: NFTs Received ───────────────────────────────────────────────────

function NftsSection({ bids, network }: { bids: BidRow[]; network: string }) {
...
function RefundsSection({ bids, network }: { bids: BidRow[]; network: string }) {
...
// ─── SECTION: Creator Sales ───────────────────────────────────────────────────

function SalesSection({ sales, network }: { sales: SaleRow[]; network: string }) {
  return (
    <div>
      <SectionHeader
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      ...
      try {
        ...
        // Fetch auctions created by this wallet (creator sales)
        const { data: salesData } = await supabase
          .from('auctions')
          .select('id, title, image_url, nft_mint, winning_bid, currency, winner_wallet, status, created_at, settle_tx, creator_payment_pending')
  ...
  return (
    <div className="glass border border-[rgba(255,255,255,0.07)] rounded-2xl p-5 mt-8">
      ...
      {loading ? (
        ...
        <>
          {tab === 'bids' && <BidsSection bids={bids} network={network} />}
          {tab === 'nfts' && <NftsSection bids={bids} network={network} />}
          {tab === 'refunds' && <RefundsSection bids={bids} network={network} />}
          {tab === 'sales' && <SalesSection sales={sales} network={network} />}
        </>
      )}
    </div>
  )
