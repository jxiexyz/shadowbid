...
export async function placeBid(
  ...
  try {
    ...
    // ── On-chain: send to escrow ──────────────────────────────────────────
    // USDC → MagicBlock Private Ephemeral Rollup (TEE, bid amount hidden on-chain)
    // SOL  → direct escrow transfer
...
/**
 * Settle an auction: pick winner, transfer NFT, pay creator, refund losers.
 * All escrow-signed transfers are handled by Supabase Edge Functions
 * (escrow private key stays server-side, never in browser).
 */
...
export async function settleAuction(auctionId: string, reveals: RevealData[] = []): Promise<SettleResult> {
  try {
    ...
    // Get auction info (need mint address + currency + seller wallet)
    ...
    if (!bids || bids.length === 0) {
      // No bids: refund NFT back to seller
    ...
    // 2. Transfer winning bid (USDC/SOL) escrow → creator/seller
    if (auction?.seller_wallet && winner.amount) {
      try {
        if (winner.currency === 'USDC') {
          await transferUsdcToCreator(auction.seller_wallet, winner.amount)
        } else {
          await transferSolToCreator(auction.seller_wallet, winner.amount)
        }
      } catch (e) {
        console.error('[settle] Payment to creator failed:', e)
        // Mark auction as payment_pending so can retry later — do NOT throw,
        // NFT already transferred so we keep going with refunds.
        await supabase
          .from('auctions')
          .update({ creator_payment_pending: true })
          .eq('id', auctionId)
      }
    }
