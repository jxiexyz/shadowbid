...
export async function getSolBalance(walletAddress: string): Promise<number> {
  // Retry 2x supaya RPC devnet timeout tidak langsung return 0 (false negative)
...
export async function buildBidTransaction(
  ...
  // Balance check dilakukan oleh Phantom/Solflare saat sign.
  // Kita skip pre-check di sini karena RPC devnet sering timeout
  // dan return 0 padahal balance aslinya ada → false "Insufficient SOL" error.
...
// transferUsdcToCreator   → escrow sign via Supabase Edge Function (server-side key)
// transferSolToCreator    → escrow sign via Supabase Edge Function (server-side key)
// refundSolToLoser        → escrow sign via Supabase Edge Function
// refundUsdcToLoser       → escrow sign via Supabase Edge Function
//
// PENTING: semua fungsi transfer/refund dari escrow harus dipanggil
// dari Supabase Edge Function (server-side), bukan browser.
 ...
 * Dipanggil via Supabase Edge Function "settle-nft-transfer" (escrow private key di server).
 */
export async function transferNftToWinner(
  ...
  if (!data?.signature) throw new Error('Edge function did not return a signature')
  return data.signature
}

/**
 * Transfer USDC from escrow → creator/seller (winning bid payment).
 * Dipanggil via Supabase Edge Function "settle-usdc-to-creator" (escrow private key di server).
 */
export async function transferUsdcToCreator(
  creatorWallet: string,
  amountUsdc: number
): Promise<string> {
  validateEscrowWallet()
  if (!isValidPublicKey(creatorWallet)) throw new Error('Invalid creator wallet address')
  if (amountUsdc <= 0) throw new Error('Amount must be greater than 0')

  const { data, error } = await supabase.functions.invoke('settle-usdc-to-creator', {
    body: { creatorWallet, amountUsdc }
  })
  if (error) throw new Error(error.message)
  if (!data?.signature) throw new Error('Edge function did not return a signature')
  return data.signature
...
/**
 * Transfer SOL from escrow → creator/seller (winning bid payment).
 * Dipanggil via Supabase Edge Function "settle-sol-to-creator" (escrow private key di server).
 */
export async function transferSolToCreator(
  ...
  const { data, error } = await supabase.functions.invoke('settle-sol-to-creator', {
    body: { creatorWallet, amountSol }
  })
  if (error) throw new Error(error.message)
  if (!data?.signature) throw new Error('Edge function did not return a signature')
  return data.signature
 ...
 * Dipanggil via Supabase Edge Function "settle-sol-refund" (escrow private key di server).
 */
 ...
 * Dipanggil via Supabase Edge Function "settle-usdc-refund" (escrow private key di server).
 */
